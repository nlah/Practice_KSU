...
namespace test2.Models
{
    ...
    public class Ad : neo4j_s
    {
        ...
        public bool create(string id_user, string name_types_standart, List<string> name_tags, string header, string data)
        {
            bool test = true;
            using (var session = db_Driver.Session().BeginTransaction())
            {
                var id = session.Run("match(n:User{Id:'" + id_user + "'})  with n Create(a:ad{time:timestamp(),header:'" + header + "',data:'" + data + "'}) merge (n)-[:product]->(a) return a");
